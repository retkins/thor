...
This software package provides a means of performing the *approximate* calculation in $O(N \space log(N))$, which provides orders of magnitude increase in the speed of the calculation while maintaining acceptable error. 
...
### Acceptable Error
In particular, this software package is concerned with *approximate* (sometimes called "low-accuracy") problems, where some small error is acceptable. Acceptable error for `thor` is defined as a deviation < 1% from the equivalent fully-integrated Biot Savart Law solution. 
