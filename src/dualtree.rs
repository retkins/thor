...
use crate::math::{
    cross, distance, mag, running_average, sort_by_indices, vec_distance
};
...
impl Targets {
    ...
    pub fn from_target_points(targets: (&[f64], &[f64], &[f64]), max_depth: u8) -> Self {
        ...
        Self {codes: codes, x: x, y: y, z: z, bx: bx, by: by, bz: bz, bbox: Some(bbox), original_indices: original_indices}
    }
...
impl TargetOctree { 
    ...
    fn add_node(
        ...
        if end - start <= leaf_threshold as usize {
            ...
            nodes.push(
                ...
                TargetNode::Leaf { level: level, target_range: (start as u32, end as u32), centroid: [cx, cy, cz]}
            );
        ...
        // Make a branch node
            ...
            nodes.push(
                TargetNode::Branch { level: level, size: size, children: [0; 8], centroid: [0.0; 3], n_targets: 0, b: [0.0; 3]}
            );
            ...
            while cursor < end {
                ...
                let child_end = get_range_in_same_node(&targets.codes, level+1, max_depth, cursor);

                // index in nodes array
                let child_idx = Self::add_node(targets, nodes, max_depth, leaf_threshold, cursor, child_end, level+1);
                ...
                match nodes[idx as usize] {
                    ...
                    TargetNode::Leaf { level: _, target_range, .. } => {
                        for i in (target_range.0 as usize)..(target_range.1 as usize)  {
                            ...
                            n_targets_parent = running_average(n_targets_parent, &mut parent_centroid, 1, &leaf_centroid);
                        }        
                    }
                }
            }
    ...
    fn propagate(&mut self, node_index: u32, b_contribution: [f64; 3]) {
        match self.nodes[node_index as usize] {
            TargetNode::Branch { level: _, size: _, children, centroid: _, b, n_targets: _ } => {
                for child in children {
                    if child < 1 {
                        ...
                        let b_total = [b_contribution[0] + b[0], b_contribution[1] + b[1], b_contribution[2] + b[2]];
                        self.propagate(child, b_total);
                    }
                }
            }, 
            TargetNode::Leaf { level: _, target_range, centroid: _} => {
                for i in (target_range.0 as usize)..(target_range.1 as usize) {
...
pub fn bfield_dualtree(
    ...
    let source_tree: SourceOctree = SourceOctree::from_source_points(
        (centx, centy, centz), 
        vol, 
        (jx, jy, jz), max_depth, leaf_threshold
    ).unwrap();

    let mut target_tree: TargetOctree = TargetOctree::from_target_points(
        (x, y, z), 
        max_depth, 
        leaf_threshold
    );

    bfield_trees(&source_tree, &mut target_tree, 0, 0, theta_source, theta_target);
...
fn bfield_trees(source_tree: &SourceOctree, target_tree: &mut TargetOctree, source_node_idx: u32, target_node_idx: u32, theta_source: f64, theta_target: f64) {
    ...
    match (source_node, target_node) {
        ...
        (&SourceNode::Leaf { level: _, source_range, centroid: _, .. }, &mut TargetNode::Leaf { level: _, target_range , centroid: _}) => {
            bfield_leaf_leaf(source_range, target_range, &source_tree.sources, &mut target_tree.targets);
        }, 

        // Source branch to target leaf 
        (&SourceNode::Branch { level: _, size: src_size, children: src_children, centroid: src_centroid, vj }, &mut TargetNode::Leaf { level: tgt_level, target_range , centroid: tgt_centroid}) => {
            ...
            if src_size / d < theta_source && tgt_size / d < theta_target {
                ...
                for i in src_children {
                    ...
                    bfield_trees(source_tree, target_tree, i, target_node_idx, theta_source, theta_target); 
                }
            }
        }

        // Source leaf to target branch 
        (&SourceNode::Leaf { level: src_level, source_range, centroid: src_centroid }, &mut TargetNode::Branch { level: tgt_level, size: _, children: tgt_children, centroid: tgt_centroid, ref mut b, n_targets:_ }) => {
            let d = distance(&src_centroid, &tgt_centroid);
            let src_size = size_at_level(source_tree.sources.bbox.as_ref().unwrap().side_length, src_level);
            let side_length = target_tree.targets.bbox.as_ref().unwrap().side_length;
            let tgt_size: f64 = size_at_level(side_length, tgt_level);
            if src_size / d < theta_source && tgt_size / d < theta_target { //src_size.max(tgt_size) / d < theta
                let b_contribution = bfield_leaf_branch(source_range, &&source_tree.sources, &tgt_centroid);
                ...
                for i in  tgt_children{
                    ...
                    bfield_trees(source_tree, target_tree, source_node_idx, i, theta_source, theta_target); 
                }
            }
        }, 

        // Source branch to target branch 
        (&SourceNode::Branch { level: _, size: src_size, children: src_children, centroid: src_centroid, vj }, &mut TargetNode::Branch { level: _, size: tgt_size, children: tgt_children, centroid: tgt_centroid, ref mut b, n_targets: _ }) => {
            let d = distance(&src_centroid, &tgt_centroid);
            if src_size / d < theta_source && tgt_size / d < theta_target {
...
fn bfield_leaf_leaf(source_range: (u32, u32), target_range: (u32, u32), sources: &Sources, targets: &mut Targets) {
...
fn bfield_branch_leaf(centroid: &[f64; 3], vj: &[f64; 3], target_range: (u32, u32), targets: &mut Targets) {
    for i in (target_range.0 as usize)..(target_range.1 as usize) {
...
fn bfield_leaf_branch(source_range: (u32, u32), sources: &Sources, tgt_centroid: &[f64;3]) -> [f64;3] {
    let (is0, is1) = (source_range.0 as usize, source_range.1 as usize);
    let centroids = (
        &sources.xg[is0..is1],
        &sources.yg[is0..is1],
