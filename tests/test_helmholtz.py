...
def test_helmholtz():
    ...
    f_upper_error = np.linalg.norm(foctree_upper - fdirect_upper) / np.linalg.norm(
        fdirect_upper
    )
    f_lower_error = np.linalg.norm(foctree_lower - fdirect_lower) / np.linalg.norm(
        fdirect_lower
    )
    ...
    axis_error = (
        2
        * np.abs(bdirect_axis - boctree_axis)
        / (np.abs(bdirect_axis) + np.abs(boctree_axis))
    )
    fig2 = plt.figure()
