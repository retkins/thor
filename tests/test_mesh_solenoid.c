...
const char* input_file = "data/solenoid_dense.csv";
...
int main() {
    ...
    Line *line = new_line(X, 100, 500, n_targets);
    clock_t start = clock(); 
    bfield_direct(centx, centy, centz, vol, jx, jy, jz, n_sources, line->x, line->y, line->z, n_targets, line->Bx, line->By, line-> Bz, 1);
    clock_t end = clock(); 
    double elapsed_direct = (double)(end - start)/CLOCKS_PER_SEC;
    printf("Direct sum time: %.3f sec\n", elapsed_direct);
    Line *line2 = new_line(X, 100, 500, n_targets);
