...
targets_axis[:,0] = np.linspace(0, 0.25, ntargets_axis)
...
print("Times: ")
print(f"\tDirect solution time:       {direct_pt_elapsed*1e3:.3f} ms")
...
fig = plt.figure()
ax = fig.add_subplot()
ax.plot(targets_axis[:,0], bdirect_pt_axis[:,2], 'k', label="Direct (pt)")
ax.plot(targets_axis[:,0], bdirect_tet_axis[:,2], 'b--', label="Direct (tet)")

ax.plot(targets_axis[:,0], boctree_pt_axis[:,2], 'r.', label="Octree (pt)")
ax.plot(targets_axis[:,0], boctree_tet_axis[:,2], 'g^', label="Octree (tet)")
