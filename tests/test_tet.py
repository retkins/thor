...
def test_tet():
    ...
    # load mesh
    nodes, centroids, vol = thor.mesh.mesh_step_tets(
        "tests/data/solenoid.stp", min_size, max_size
    )
    n = len(vol)
    ...
    bdirect_tet_axis = thor.bfield_tetrahedrons_direct(
        nodes, centroids, vol, jdensity, targets_axis, nthreads=nthreads
    )
    boctree_tet_axis = thor.bfield_tetrahedrons(
        nodes,
        centroids,
        vol,
        jdensity,
    ...
    boctree_pt_axis = thor.bfield_octree(
        centroids, vol, jdensity, targets_axis, nthreads=nthreads, theta=theta_pt
    )
    ...
    print(
        f"\tDirect (pt) solution:  ({bdirect_pt_axis[i, 0]:.6f}, {bdirect_pt_axis[i, 1]:.6f}, {bdirect_pt_axis[i, 2]:.6f})"
    )
    print(
        f"\tDirect (tet) solution: ({bdirect_tet_axis[i, 0]:.6f}, {bdirect_tet_axis[i, 1]:.6f}, {bdirect_tet_axis[i, 2]:.6f})"
    )
    print(
        f"\tOctree (pt) solution:  ({boctree_pt_axis[i, 0]:.6f}, {boctree_pt_axis[i, 1]:.6f}, {boctree_pt_axis[i, 2]:.6f})"
    )
    print(
        f"\tOctree (tet) solution: ({boctree_tet_axis[i, 0]:.6f}, {boctree_tet_axis[i, 1]:.6f}, {boctree_tet_axis[i, 2]:.6f})"
    )
    ...
    start = perf_counter()
    bdirect_pt = boctree = thor.bfield_direct(
    ...
    start = perf_counter()
    bdirect_tet = thor.bfield_tetrahedrons_direct(
        nodes, centroids, vol, jdensity, targets, nthreads=nthreads
    )
    end = perf_counter()
    direct_tet_elapsed = end - start

    start = perf_counter()
    boctree_pt = thor.bfield_octree(
        centroids, vol, jdensity, targets, nthreads=nthreads, theta=theta_pt
    )
    end = perf_counter()
    octree_pt_elapsed = end - start

    start = perf_counter()
    boctree_tet = thor.bfield_tetrahedrons(
        nodes, centroids, vol, jdensity, targets, theta=theta_tet, nthreads=nthreads
    )
    end = perf_counter()
    ...
    print(
        f"Mean fields error within the mesh (pt/direct sources):  {err_mesh_pt_direct * 100:.2}%"
    )
    print(
        f"Mean fields error within the mesh (pt/octree sources):  {err_mesh_pt * 100:.2}%"
    )
    print(
        f"Mean fields error within the mesh (tet/octree sources): {err_mesh_tet * 100:.2}%"
    )
