...
def bfield_direct(
    ...
    Args:
        centroids: [m] Nx3 array, location of source element centroids in 3D (cartesian)
                   space
...
def bfield_octree(
    ...
    Args:
        ...
        targets: [m] Nx3 array, describes location of target points at which to
                 calculate magnetic fields
        theta: angle-opening parameter for Barnes-Hut; smaller values correspond to
               lower error at the expense of longer runtimes. Suggested values in
...
def bfield_dualtree(
    ...
    """Compute the magnetic field generated by a collection of current-carrying
        finite elements at `targets` using the dual Barnes-Hut (octree)
        Biot-Savart method.
    ...
    Args:
        ...
        targets: [m] Nx3 array, describes location of target points at which to
                 calculate magnetic fields
                 theta_source, theta_target: angle-opening parameter for Barnes-Hut;
                 smaller values correspond to lower error at the expense of longer
                 runtimes. Suggested values in the range of [0.1, 0.5]
        leaf_threshold: how many individual sources to aggregate per leaf
        nthreads: number of threads to use for the calculation (default: all available)
...
def bfield_hexahedron(
    nodes: NDArray[float64], jdensity: NDArray[float64], target: NDArray[float64]
) -> NDArray[float64]:
...
def hfield_dipole(
    ...
    Args:
        ...
        targets: [m] Nx3 array, describes location of target points at which to
                 calculate magnetic fields
        theta: angle-opening parameter for Barnes-Hut; smaller values correspond to
               lower error at the expense of longer runtimes. Suggested values in the
               range of [0.1, 0.5]
        leaf_threshold: how many individual sources to aggregate per leaf
        nthreads: number of threads to use for the calculation (default: all available)
