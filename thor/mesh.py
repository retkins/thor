    ...
    try:
        ...
        with open(outfile, "w") as f:
            ...
            for elem_type, elem_tags, elem_nodes in zip(
                element_types, element_tags, node_tags, strict=False
            ):
                elem_name, dim, order, num_nodes, local_coords, num_primary_nodes = (
                ...
                for _elem_tag, nodes in zip(elem_tags, elem_nodes, strict=False):
                    ...
                    if "Tetrahedron" in elem_name:
                        ...
                        print(
                            f"Warning: Unknown element type {elem_name},"
                            "approximating volume"
                        )
                        volume = 0.0

                    f.write(
                        f"{centroid[0]:.6f},{centroid[1]:.6f},{centroid[2]:.6f},{volume:.10e}\n"
                    )
        ...
        print(
            "Error - gmsh is not installed."
            f"Could not process elements in file `{infile}`"
        )
